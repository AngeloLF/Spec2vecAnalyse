...
def makePlotAnalyse(ana, score, idec=0.1):
    ...
    for k, a in ana.k2a.items():
        ...
        dfmean = np.mean(ana.k2t[k]["tab"], axis=0)
        dfmin  = np.min(ana.k2t[k]["tab"], axis=0)
...
def generate_html_table(colonnes, ligness, text, y, sorting=False, marker='.', savefig_name=None, markers=None, colors=None, score=None):
    ...
    if sorting:
        ...
        lignes4graph = [ligness[i] for i in index if not ("cal" in ligness[i] and not "wc" in ligness[i])]
